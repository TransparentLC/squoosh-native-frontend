  ...
  return {
    ...
    async load(id) {
      ...
      if (id.startsWith(sourcePrefix)) {
        // Error: Could not load css · Issue #1204 · GoogleChromeLabs/squoosh
        // https://github.com/GoogleChromeLabs/squoosh/issues/1204
        const path = (nomalizePath(id.slice(sourcePrefix.length)).startsWith('\\') ? (resolvePath(__dirname).split(':', 2)[0] + ':') : '') + nomalizePath(id.slice(sourcePrefix.length));
